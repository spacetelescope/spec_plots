    ...
    for i,s in enumerate(subplot_segment_names):
        ...
        if is_bigplot:
            ...
            if stitched_spectrum is not None:
                ...
                raise SpecUtilsError("You must provide a stitched spectrum through the `stitched_spectrum` input argument if creating a small (thumbnail-sized) plot.")
...
##                    this_figure.suptitle(os.path.basename(cos_spectrum.orig_file) + ": " + ','.join(['{0:6.2f}'.format(y) for y in covering_fractions]), fontsize=18, color='white')
            ...
            if debug:
                ...
                debug_oplot(this_plotarea, "cos", all_wls, all_fls, all_flerrs, all_dqs, plot_metrics[i]["median_flux"], plot_metrics[i]["median_fluxerr"], flux_scale_factor, fluxerr_scale_factor, plot_metrics[i]["fluxerr_95th"])
